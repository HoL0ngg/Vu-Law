...
- A newer Client document exists: `Feedbacks/IVL _ WEBSITE content_updated.docx`. Beyond applying the full-name change itself, it adds **Vietnamese copy for Phase 4 (People) and the Contact page**, which are still English on `/vi`. Not yet transcribed. Note that its English sections write the firm as "Integritas Vũ Legal" with the diacritic, which contradicts the Client's own instruction that English uses "Integritas Vu Legal"; AGENTS.md rule 8 and that instruction were followed instead.
...
- Vietnamese coverage: complete for Homepage, About Us and Expertise, plus navigation, page labels, the Insights heading and the footer. The `Home` nav label is `Trang chủ`, approved by the site owner rather than supplied by the Client; confirm it at sign-off. Still English on `/vi`: People, Contact and Careers page copy, all page `<title>` metadata, and the `Experience` page label (the Client's Vietnamese list omits it, and whether Experience should exist at all is open question 1). The site owner has confirmed that publishing Vietnamese in this partial state is intended.
