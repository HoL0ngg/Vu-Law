...
- Pages with Client content: `/` (Phase 1), `/about` (Phase 2), `/expertise` (Phase 3, eight practice areas as in-page anchors), `/people` (Phase 4), `/contact` (Phase 5, including the enquiry form), `/insights` (Phase 6, category cards only), `/careers` (Phase 7). Still label-only `noindex` placeholders: `/experience`, `/terms-of-use`, `/privacy-policy`, `/legal-disclaimer`.
