...
- Client inputs now supplied (2026-09): the firm's office address (English and Vietnamese), telephone `0938 170 130` and `contact@integritasvulegal.com`, shown in the footer contact column and in a firm card on `/contact`, with the telephone and email as `tel:` / `mailto:` links. Portraits for both lawyers live in `public/images/`, renamed to ASCII slugs matching their routes. Mr Vu's `Direct:` line and email on `/contact`, and the Careers application address, now carry the firm's switchboard and general inbox at the site owner's direction; the Client's document still lists all three as unfilled placeholders, so the attribution needs confirming at sign-off. Only `[LinkedIn Profile]` remains blank.
