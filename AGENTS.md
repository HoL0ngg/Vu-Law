...
- Vietnamese coverage: complete for Homepage, About Us and Expertise, plus navigation, page labels, the Insights heading and the footer. Still English on `/vi`: People, Contact and Careers page copy, all page `<title>` metadata, the `Home` nav label and the `Experience` page label (the Client's Vietnamese lists omit both).
