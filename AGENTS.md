...
- Routing convention: App Router with locale route groups. `app/(en)/` serves English at unprefixed paths; `app/(vi)/vi/[[...segments]]/` mirrors the same slugs under `/vi` and is gated by `VI_ENABLED`. Shared views (`HomePageView`, `AboutPageView`, `PlaceholderPage`) are locale-agnostic and receive a dictionary plus a `locale`. Hrefs are built by `routePath()` / `aboutAnchors` in `app/i18n/routes.ts`, never as scattered literals.
- Prototype routes/pages found, and how each maps to the sitemap (kept / renamed / removed / unlinked): `/` kept as Home, rebuilt with the client's copy. `/publications` removed (`app/publications/page.tsx` deleted); it was prototype-only and not in the sitemap, and `/insights` now represents that part of the sitemap. Prototype header destinations mapped: `#about` -> `/about`, `#services` -> `/expertise`, `/publications` -> `/insights`, `#contact` -> `/contact`, logo `#top` -> `/`. Created: `/about`, `/expertise`, `/people`, `/insights`, `/careers`, `/contact`, plus `/experience` (open question 1) and `/terms-of-use`, `/privacy-policy`, `/legal-disclaimer`. All except `/` and `/about` are minimal placeholders carrying only their page label, marked `noindex`.
