...
- Locale routing decision (default: English at `/`, Vietnamese reserved at `/vi`): English remains unprefixed. Mirrored `/vi` routing is reserved behind `VI_ENABLED = false`; while disabled `/vi/*` returns 404 (verified) and no switcher, `hreflang` or Vietnamese sitemap entry is rendered. The Client supplied Vietnamese copy for Phases 1-3 in `Feedbacks/IVL _ WEBSITE content.docx`; it is transcribed into `vi.ts`. People, Contact, Careers and Insights have no Vietnamese yet, so the flag stays off.
